...
<html lang="es">
<head>
    <x-keiyi-head title="Registro Profesor — Keiyi Digital" description="Regístrate como profesor en Keiyi Digital para ofrecer cursos grupales." />
</head>
<body>
    <x-keiyi-nav />

    <section style="min-height: 80vh; display: flex; align-items: center; justify-content: center; padding: 48px 20px;">
        <div style="max-width: 480px; width: 100%; font-family: 'Space Grotesk', sans-serif;">
            ...
            <div style="text-align: center; margin-bottom: 32px;">
                ...
                <p style="color: #555; margin-top: 8px; font-size: 14px;">Registro de Profesor</p>
            </div>

            <div style="border: 3px solid #000; padding: 32px; box-shadow: 6px 6px 0 #000; background: #fff;">

                <form method="POST" action="{{ route('registro.profesor.store') }}" style="display: flex; flex-direction: column; gap: 14px;">
                    @csrf
                    ...
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px;">
                        <div>
                            ...
                            <input id="apellido_paterno" type="text" name="apellido_paterno" value="{{ old('apellido_paterno') }}" required
                                style="width: 100%; padding: 12px; border: 2px solid #000; font-family: 'Space Grotesk', sans-serif; font-size: 14px; box-sizing: border-box;">
                            @error('apellido_paterno') <p style="color: #dc2626; font-size: 12px; margin-top: 4px;">{{ $message }}</p> @enderror
                        </div>
                        <div>
                    ...
                    <div>
                        <label for="company_name" style="font-size: 13px; font-weight: 700; display: block; margin-bottom: 4px;">Institución / Empresa</label>
                        <input id="company_name" type="text" name="company_name" value="{{ old('company_name') }}"
                            placeholder="Nombre de tu escuela, empresa u organización"
                            style="width: 100%; padding: 12px; border: 2px solid #000; font-family: 'Space Grotesk', sans-serif; font-size: 14px; box-sizing: border-box;">
                    ...
                    <div>
                        <label for="student_limit" style="font-size: 13px; font-weight: 700; display: block; margin-bottom: 4px;">Cantidad de alumnos</label>
                        <select id="student_limit" name="student_limit" required
                            style="width: 100%; padding: 12px; border: 2px solid #000; font-family: 'Space Grotesk', sans-serif; font-size: 14px; box-sizing: border-box; background: #fff;">
                            <option value="">Selecciona...</option>
                            <option value="5" {{ old('student_limit') == '5' ? 'selected' : '' }}>5 alumnos</option>
                            <option value="10" {{ old('student_limit') == '10' ? 'selected' : '' }}>10 alumnos</option>
                            <option value="15" {{ old('student_limit') == '15' ? 'selected' : '' }}>15 alumnos</option>
                            <option value="20" {{ old('student_limit') == '20' ? 'selected' : '' }}>20 alumnos</option>
                            <option value="25" {{ old('student_limit') == '25' ? 'selected' : '' }}>25 alumnos</option>
                        </select>
                        @error('student_limit') <p style="color: #dc2626; font-size: 12px; margin-top: 4px;">{{ $message }}</p> @enderror
                    </div>
